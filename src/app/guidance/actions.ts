...
const guidanceFormSchema = z.object({
  profile: z.string().min(1, "Profile information is optional but helpful.").or(z.literal("")).optional(), // Allow empty string or make truly optional
...
export async function handleGenerateGuidance(
  ...
  const validatedFields = guidanceFormSchema.safeParse({
    profile: formData.get('profile') || "", // Ensure profile is at least an empty string
  ...
  // Refine profile validation: If profile is empty, don't apply min length
  if (validatedFields.data.profile && validatedFields.data.profile.length > 0 && validatedFields.data.profile.length < 50) {
     return {
      message: "Invalid form data. Profile, if provided, should be at least 50 characters.",
      fields: { profile: "Profile, if provided, should be detailed, at least 50 characters." },
      isError: true,
    };
  }
  ...
  try {
    const input: PersonalizedGuidanceInput = {
        profile: validatedFields.data.profile || "User chose not to share profile details.", // Default if empty
